#!/usr/bin/env python3
"""
Filesystem ETL Script: Migrate data from SQL Server to Apache Iceberg tables.
...
class SQLServerToIcebergETL:
    """ETL pipeline for migrating SQL Server data to Iceberg tables using filesystem storage."""
    ...
    def initialize_catalog(self) -> None:
        ...
        try:
            ...
            logger.info(f"Catalog metadata stored in SQLite at {catalog_db_path}")
            ...
            try:
                ...
                # Try to create namespace anyway
                try:
                    self.catalog.create_namespace('sales')
                    logger.info("Created 'sales' namespace in Iceberg catalog")
                except:
    ...
    def create_iceberg_table(self, table_name: str, arrow_table: pa.Table,
            ...
            for col in partition_by:
                ...
                for field in iceberg_fields:
                    if field.name == col:
                        ...
                        if 'date' in str(field.field_type).lower() or 'timestamp' in str(field.field_type).lower():
        ...
        try:
            ...
            # Create table with filesystem location
            table_location = os.path.join(self.warehouse_path, namespace, table_name)
            ...
            logger.info(f"Created Iceberg table {full_table_name} at {table_location}")
            return table
    ...
    def write_to_iceberg(self, table: Table, arrow_table: pa.Table) -> None:
        ...
        try:
            ...
            output_path = os.path.join(location, 'data')
            os.makedirs(output_path, exist_ok=True)
            
            # Generate unique file name
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            file_path = os.path.join(output_path, f"part-00000-{timestamp}.parquet")
            ...
            # Note: In production, you would also update the Iceberg metadata
            # PyIceberg 0.9.1 has evolving write support
    ...
    def migrate_table(self, schema: str, table_name: str, 
        ...
        try:
            ...
            # Determine partition info for display
            partition_info = ""
            if partition_by:
                partition_info = f" [partitioned by {', '.join(partition_by)}]"
    ...
    def run_migration(self) -> None:
        ...
        logger.info("="*60)
        logger.info("Starting SQL Server to Iceberg migration (Filesystem Mode)")
        logger.info("="*60)
        ...
        for result in results:
            if result['status'] == 'success':
                ...
                logger.info(f"✓ {result['table']}: {result['rows_migrated']} rows{partition_info}")
        ...
        logger.info(f"Data stored in: {self.warehouse_path}")
